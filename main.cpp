...
//    Function& func = *(Function*) data;
    energydata& edata = *(energydata*)data;
    Function& func = edata.func;
    vector<DMatrix> res = func(vector<DMatrix>{x, DMatrix(edata.t)});
    grad = res[0].nonzeros();
    return res[1].toScalar();
}

gsresults initialize(double Wi, double Wf, double mu, vector<double>& xi, int ngrid) {
    ...
    SXFunction nlp("nlp", nlpIn("x", f, "p", t), nlpOut("f", E, "g", g));
    ...
    SXFunction nlpnorm("nlpnorm", nlpIn("x", f, "p", t), nlpOut("f", Enorm));
    ...
    vector<double> Es;
        ...
        progress_display progress(grid.size());

        energydata data = {0, grad};
    ...
    for (double ti : grid) {
        ...
        double E0ipopt = nlpnorm(vector<DMatrix>{x0ipopt, DMatrix(ti)})[0].toScalar(); //res["f"].toScalar();
...
//        lopt.set_min_objective(energymin, &grad);
        lopt.set_min_objective(energymin, &data);
        double E0nlopt;
...
//        cout << "Enlopt = " << E0nlopt << endl;
        ...
        vector<vector<complex<double>>> fsi;
        for (int i = 0; i < L; i++) {
            vector<complex<double>> fsii;
            for (int n = 0; n <= nmax; n++) {
                fsii.push_back(complex<double>(x0[2 * (i * dim + n)], x0[2 * (i * dim + n) + 1]));
            }
...
void worker(worker_input* input, worker_tau* tau_in, worker_output* output, managed_shared_memory& segment) {//std::string tau_name, std::string output_name, managed_shared_memory& segment) {
    ...
    //    SX t = SX::sym("t");
    //
    //    SX Wt = if_else(t < 1, Wi + (Wf - Wi) * t / (1), Wf + (Wi - Wf) * (t - 1) / (1));
    //
    //    U0 = UW(Wt);
    //    for (int i = 0; i < L; i++) {
    //        J[i] = JWij(Wt * xi[i], Wt * xi[mod(i + 1)]);
...
//        //        managed_shared_memory segment(create_only, "SharedMemory", size);
        ...
        gsresults gsres = initialize(Wi, Wf, mui, xi, ngrid);
        printMath(os, "tsres", resi, gsres.ts);
        printMath(os, "E0sres", resi, gsres.Es);
        printMath(os, "f0sres", resi, gsres.fs);
